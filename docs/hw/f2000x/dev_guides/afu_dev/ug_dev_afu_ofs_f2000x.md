# Intel<sup>&reg;</sup> Accelerator Functional Unit Developer Guide: Open FPGA Stack for Intel® Agilex® FPGAs SoC Attach
...
You are strongly encouraged to review the [FPGA Interface Manager Developer Guide: Open FPGA Stack for Intel® Agilex® FPGAs SoC Attach](https://github.com/intel-innersource/applications.fpga.ofs.opensource-documentation/blob/main/docs/hw/f2000x/dev_guides/fim_dev/ug_dev_fim_ofs.md)
...
- Building the SoC Attached FIM
- Generating a relocatable AFU build-tree or build-template from the SoC Attached FIM.
- Compiling the host_chan_mmio example AFU for the SoC Attached FIM.

Additionally, this section includes steps to demonstrate loading and running the host_chan_mmio example AFU in an F2000x equipped Linux server.
...
This guide uses Intel IPU Platform F2000X-PL as the target OFS compatible FPGA PCIe card for demonstration steps.  The Intel IPU Platform F2000X-PL must be fully installed following the [Intel® F2000X-PL IPU SoC Attach Bring Up Guide](https://github.com/intel-innersource/applications.fpga.ofs.opensource-documentation/blob/main/docs/hw/f2000x/user_guides/ug_soc_attach/ug_soc_attach.md). If using a different OFS FPGA PCIe card, contact your supplier for instructions on how to install and operate user developed AFUs.
...
- Download Intel<sup>&reg;</sup> Quartus<sup>&reg;</sup> Prime Pro Edition Version 23.1 from [Quartus Pro Prime Download](https://www.intel.com/content/www/us/en/software-kit/727906/intel-quartus-prime-pro-edition-design-software-version-23-1-for-linux.html).
...
```sh
$ ls
ipss  LICENSE.txt  ofs-common  sim  src  syn  tools  verification
```
...
Follow the instructions in the Getting Started Guide: Open FPGA Stack for Intel IPU Platform F2000X-PL, section [4.0 OPAE Software Development Kit](https://github.com/intel-innersource/applications.fpga.ofs.opensource-documentation/blob/main/docs/hw/n6001/user_guides/ug_qs_ofs_n6001/ug_qs_ofs_n6001.md#40-opae-software-development-kit) to build and install the required OPAE SDK for the Intel IPU Platform F2000X-PL.

Working with the Intel® Intel IPU Platform F2000X-PL card requires **opae-2.5.0-3**. Follow the instructions in the Getting Started Guide: Intel® Open FPGA Stack for Intel IPU Platform F2000X-PL section [4.0 OPAE Software Development Kit](https://github.com/intel-innersource/applications.fpga.ofs.opensource-documentation/blob/main/docs/hw/n6001/user_guides/ug_qs_ofs_n6001/ug_qs_ofs_n6001.md#40-opae-software-development-kit). However, just make sure to check out the cloned repository to tag **2.5.0-3** and branch **release/2.5.0**.
...
The documentation in the [ofs-platform-afu-bbb](https://github.com/OFS/ofs-platform-afu-bbb.git) and [intel-fpga-bbb](https://github.com/OPAE/intel-fpga-bbb.git) repositories further address
  - The PIM concept.
...
> Note: For instructions to compile FIMs available for the Intel IPU Platform F2000X-PL, refer to [FPGA Interface Manager Developer Guide: Open FPGA Stack for Intel® Agilex® FPGAs SoC Attach](https://github.com/intel-innersource/applications.fpga.ofs.opensource-documentation/blob/main/docs/hw/f2000x/dev_guides/fim_dev/ug_dev_fim_ofs.md). 
...
$ export OPAE_PLATFORM_ROOT=$OFS_ROOTDIR/work_base_pr/pr_build_template
...
syn/build_top.sh [-p] target_configuration work_dir 
    ...
    * target_configuration - Specifies the project, FIM and board for this script run.  
              ...
              For example: f2000x/base/adp:A1
              
        * <OFS_PROJECT> is the high level project.  For this release, F2000x is supported by the provided files.
        * <OFS_FIM> the name of the FIM to built.  This name corresponds to FIM directory name containing source 
              files.  For this release, base is supported by the provided files.
    ...
    * work_dir - Work Directory for this build in the form a directory name. 
                   It is created in the <local repo directory>/ofs-f2000x-pl/<work_dir> 
          - NOTE: The directory name must start with "work".  If the work directory exists,
                  ...
                  - e.g. 
                    ...
                    syn/build_top.sh f2000x/base/adp:A1 work_base
                
                A work directory as a name will be created in <local repo directory>/ofs-f2000x-pl/work_base
                
                The omission of <work_dir> results in a default work directory 
                    (<local repo directory>/ofs-f2000x-pl/work)

        - compile reports and artifacts (.rpt, .sof, etc) are stored in 
            <work_dir>/syn/<OFS_PROJECT>/<OFS_FIM>/<OFS_BOARD>/syn_top/output_files
  
        - log file created in ofs-f2000x-pl directory
...
The next example command, builds the SoC Attached fFIM or the Intel IPU Platform F2000X-PL and generates the relocatable PR build-tree in ```$OFS_ROOTDIR/work_base_pr/pr_build_template```.

```sh
# Build the provided base example design:
$ cd $OFS_ROOTDIR
    
$ ofs-common/scripts/common/syn/build_top.sh -p f2000x/base/adp work_base_pr


  ... build takes ~5 hours to complete

Compile work directory:     <$OFS_ROOTDIR>/work_base_pr/syn/syn_top
Compile artifact directory: <$OFS_ROOTDIR>/work_base_pr/syn/syn_top/output_files
...
```sh
$ cd $OFS_ROOTDIR
$ ofs-common/scripts/common/syn/build_top.sh -p f2000x/base/adp:null_he,null_he_hssi,null_he_mem,null_he_mem_tg work_adp_null_he
```
...
In this step, you will load the previously compiled SoC Attached FIM binary into the flash of the Intel IPU Platform F2000X-PL board. AFUs developed in this guide using the SoC Attached FIM and the generated build-tree will be compatible with the image loaded on the Intel IPU Platform F2000X-PL board.

```sh
# On Development Host
$ cd $OFS_ROOTDIR/work_base_pr/syn/syn_top/output_files
...
The build steps presented below demonstrate the ease in building and running an actual AFU on the Intel IPU Platform F2000X-PL board. To successfully execute the instructions in this section, you must have set up your development environment and compiled the SoC Attached FIM as instructed in section 2 of this document.

Additionally, you need to set the ```OPAE_PLATFORM_ROOT``` environment variable to the path where the PR build-tree was generated during the SoC Attached FIM compilation through the [-p] switch.
...
Execute ```afu_synth_setup``` as follows to create the synthesis environment for a ```host_chan_mmio``` AFU that fits the SoC Attached FIM previously constructed.

```sh
$ cd $OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio

$ mkdir -p $OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio/f2000x/base
$ cd $OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio/f2000x/base
$ afu_synth_setup -s $OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio/hw/rtl/test_mmio_axi1.txt hardware


Now, move into the synthesis environment ```hardware``` directory just created. From there, execute the ```afu_synth``` command. The successful completion of the command will produce the ```host_chan_mmio.gbs``` file under the synthesis environment directory, ```$OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio/f2000x/base/hardware```.

$ cd hardware
$ $OPAE_PLATFORM_ROOT/bin/afu_synth

Loading platform database: /home/<user_area>/ofs-X.X.X/ofs-f2000x-pl/work_base_pr/pr_build_template/hw/lib/platform/platform_db/ofs_agilex_adp.json
...
Copying build from /home/<user_area>/ofs-X.X.X/ofs-f2000x-pl/work_base_pr/pr_build_template/hw/lib/build...
...
The previous output indicates the successful compilation of the AFU and the compliance with the timing requirements. Analyze the reports generated in case the design does not meet timing. The timing reports are stored in the directory, ```$OFS_PLATFORM_AFU_BBB/plat_if_tests/host_chan_mmio/f2000x/base/hardware/build/syn/syn_top/output_files/timing_report```.
...
5. To test the AFU in actual hardware, load the ```hello_world.gbs``` to the Intel IPU Platform F2000X-PL card. For this step to be successful, the base FIM must have already been loaded to the Intel IPU Platform F2000X-PL card following the steps described in Section 2 of this document.
...
$ cd $OFS_ROOTDIR/work_base_pr/pr_build_template
...
Tool Brand: VCS
Loading platform database: /ofs-f2000x-pl/work_base_pr/build_tree/hw/lib/platform/platform_db/ofs_agilex_adp.json
...
2. Prepare an RTL simulation environment for the AXI version of the ```hello_world``` AFU.
    ...
    $ cd $OFS_BUILD_ROOT/examples-afu/tutorial/afu_types/01_pim_ifc/hello_world
    $ mkdir -p $OFS_BUILD_ROOT/examples-afu/tutorial/afu_types/01_pim_ifc/hello_world/f2000x/base
    $ cd $OFS_BUILD_ROOT/examples-afu/tutorial/afu_types/01_pim_ifc/hello_world/f2000x/base
...
3. Build and execute the AFU RTL simulator.
    ...
    ```sh 
    $ cd $OFS_BUILD_ROOT/examples-afu/tutorial/afu_types/01_pim_ifc/hello_world/f2000x/base/simulation
    $ make
    $ make sim  
    ```
...
5. Also, set the ASE_WORKDIR environment variable following the instructions given in the "hardware simulation" shell.
  ...
  ```sh
  $ export ASE_WORKDIR=$OFS_BUILD_ROOT/examples-afu/tutorial/afu_types/01_pim_ifc/hello_world/f2000x/base/simulation/work
  ```
